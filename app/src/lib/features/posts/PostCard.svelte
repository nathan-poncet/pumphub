<script lang="ts">
	...
	import { activities } from '../activities/store/activities';
...
>
	...
	<div class="relative flex items-start justify-between p-4 sm:p-6 lg:p-8">
		<div class="sm:pt-18 pt-12 text-white lg:pt-24">
			<h3 class="text-xl font-bold sm:text-2xl">{post.title}</h3>

			<p class="text-sm">{$activities.find((activity) => activity.id === post.activity)?.title}</p>
		</div>
