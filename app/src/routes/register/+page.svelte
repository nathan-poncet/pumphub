		...
		<div class="mb-4 gap-2">
			...
			<label class="label mt-4">
				...
				{#if form?.error && form.data.passwordConfirm}
					<p class="text-error-600 mt-4 text-sm">{form.data.passwordConfirm.message}</p>
				{/if}
			</label>
			{#if form?.error && Object.keys(form.data).length === 0}
				<p class="text-error-600 mt-4 text-sm">{form.message}</p>
			{/if}
			<button class="btn variant-filled mt-8">Register</button>
		</div>
	</form>
</div>
